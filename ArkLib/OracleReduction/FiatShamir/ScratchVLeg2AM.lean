...
import ArkLib.OracleReduction.FiatShamir.ScratchLazyImpl2AM

/-!
# SCRATCH (Agent 2): the LAZY verifier leg — full-transcript Fiat-Shamir coupling (#116, obligation I)
...
/-- **Full take for `extend`** (the just-inserted V_to_P round's key uses the full input messages):
taking `k` rounds of `messages.extend h` recovers `messages`. -/
theorem take_extend_self {k : Fin n} (messages : pSpec.MessagesUpTo k.castSucc)
    (h : pSpec.dir k = Direction.V_to_P) :
    (messages.extend h).take ((Fin.last k.castSucc.val).castLE
      (by simp only [Fin.val_succ, Fin.val_castSucc]; omega)) = messages := by
  rw [take_extend_castLE messages h (Fin.last k.castSucc.val)
    (by simp only [Fin.val_succ, Fin.val_castSucc]; omega), take_last]

/-- **Full take for `concat`** (the just-inserted P_to_V round's messages, before appending). -/
theorem take_concat_self {k : Fin n} (messages : pSpec.MessagesUpTo k.castSucc)
    (h : pSpec.dir k = Direction.P_to_V) (msg : pSpec.Message ⟨k, h⟩) :
    (messages.concat h msg).take ((Fin.last k.castSucc.val).castLE
      (by simp only [Fin.val_succ, Fin.val_castSucc]; omega)) = messages := by
  rw [take_concat_castLE messages h msg (Fin.last k.castSucc.val)
    (by simp only [Fin.val_succ, Fin.val_castSucc]; omega), take_last]
...
theorem deriveTranscriptFS_lazy_run_of_runOutput
    ...
    StateT.run (simulateQ
      ...
      = pure (reconstruct (StmtIn := StmtIn) stmt z.1.1 z.2.2, s, z.2.2) := by
  refine deriveTranscriptFS_lazy_run impl stmt z.1.1 s z.2.2 (fun i hDir => ?_)
...
#print axioms take_extend_self
#print axioms take_concat_self
